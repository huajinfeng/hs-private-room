...
- **结构**：使用大括号 `{}`，每个元素由 `键:值` 组成。
- **多样性**：字典的值可以是任意类型（字符串、列表、嵌套字典等）。
- **长度**：使用 `len(dct)` 获取键值对的总数。

### 2. 基本操作矩阵

针对字典的日常增删改查，文档提供了清晰的方法对比：

| **操作类型**  | **方法 / 语法**      | **特点**                                      |
| ------------- | -------------------- | --------------------------------------------- |
| **访问**      | `dct['key']`         | 简单直接，但若键不存在会报错 (KeyError)。     |
| **安全访问**  | `dct.get('key')`     | **推荐使用**，键不存在时返回 `None`，不报错。 |
| **添加/修改** | `dct['key'] = value` | 键不存在则添加，键存在则更新其值。            |
| **检查存在**  | `'key' in dct`       | 返回布尔值，判断某个键是否在字典中。          |
...
- **`pop(key)`**: 删除指定键并返回其对应的值。
- **`popitem()`**: 删除并返回字典中的最后一对键值。
- **`del dct['key']`**: 物理删除指定键值对。
- **`clear()`**: 清空字典内所有内容，保留空字典对象。

### 4. 视图对象 (View Objects)

为了方便遍历和转换，Python 提供了三个重要方法：

- **`keys()`**: 获取所有键。
- **`values()`**: 获取所有值。
- **`items()`**: 获取所有键值对（每个对以元组形式呈现），常用于 `for` 循环。

### 5. 安全复制

- **`copy()`**: 执行浅拷贝。通过 `dct.copy()` 创建新对象，可以确保对副本的修改不会影响到原始字典的数据安全性。
