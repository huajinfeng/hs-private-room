...
# JSON: 字典的字符串形式
person_json = "{'name': 'hua', 'country': 'China', 'city': 'Suzhou', 'skills': ['JavaScrip', 'React', 'Python']}"
...
# 输出
<class 'dict'>
{'name': 'hua', 'country': 'China', 'city': 'Suzhou', 'skills': ['JavaScrip', 'React', 'Python']}
hua
```
...
with open('./files/csv_example.csv', 'w', encoding='UTF8', newline='') as f:
    ...
    # 写入数据
    writer.writerow(['hua', 'China', 'Suzhou', 'JavaScript'])
```
